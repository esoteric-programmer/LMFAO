...
/**
 * Store Malbolge program as linked list of memory cells, starting with smallest one.
 */
typedef struct MemoryListElement {
	Number address;
	Number opcode;
	MemoryCell* cell;
	struct MemoryListElement* prev;
	struct MemoryListElement* next;
} MemoryListElement;

typedef struct {
	MemoryListElement* first;
	MemoryListElement* last;
} MemoryList;
