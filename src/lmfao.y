...
Offset:			OFFSET CONSTANT IgnoreEmptylines {$$.offset=$2.offset; $$.val=$2.val;};

IgnoreEmptylines:	EMPTYLINE IgnoreEmptylines |

Codeblock:		{$$ = 0;}
				| Offset LABEL Codeexpressions { if (!insert_label(0, $3, $2, &labeltree, &@2)) return 1; $$=$3; if ($$ != 0) {  $$->offset.offset = $1.offset; $$->offset.val = $1.val;}}
				| LABEL Codeexpressions { if (!insert_label(0, $2, $1, &labeltree, &@1)) return 1; $$=$2; };

Codeexpressions:	{ $$ = 0;}
				| LABEL Codeexpressions { if (!insert_label(0, $2, $1, &labeltree, &@1)) return 1; $$=$2; }
				| Codeexpression Codeexpressions { $$=(CodeBlock*)malloc(sizeof(CodeBlock)); $$->next = $2; if ($2 != 0) $2->prev = $$; $$->prev=0; $$->offset.offset = UNSET; $$->offset.val=-1; $$->command=$1; if ($2 == 0) $$->num_of_blocks = 1; else $$->num_of_blocks = $2->num_of_blocks+1; COPY_CODE_POSITION($$->code_position, @1, @1); };
...
Datablock:		{ $$ = 0; }
				| Offset LABEL Dataexpressions { if (!insert_label(1, $3, $2, &labeltree, &@2)) return 1; $$=$3; $$->offset.offset = $1.offset; $$->offset.val = $1.val; }
				| LABEL Dataexpressions { if (!insert_label(1, $2, $1, &labeltree, &@1)) return 1; $$=$2; };

Dataexpressions:	{ $$ = 0;}
				| LABEL Dataexpressions { if (!insert_label(1, $2, $1, &labeltree, &@1)) return 1; $$=$2; }
				| Dataexpression Dataexpressions { $$=(DataBlock*)malloc(sizeof(DataBlock)); $$->next = $2; if ($2 != 0) $2->prev = $$; $$->prev=0; $$->offset.offset = UNSET; $$->offset.val=-1; $$->data=$1; if ($2 == 0) $$->num_of_blocks = 1; else $$->num_of_blocks = $2->num_of_blocks+1; COPY_CODE_POSITION($$->code_position, @1, @1); }
...
if (length <= 0){
  ...
  for (i=0;i<length;i++){
    ...
    if ($1[i+1] == '\\'){
      i++;
      tmp->data->leaf_element->number.offset = T0;
      tmp->data->leaf_element->number.val = (long long)((unsigned char)$1[i+1]);
      switch ($1[i+1]){
        case 'n':
          tmp->data->leaf_element->number.val = (long long)((unsigned char)'\n');
          break;
        case 'r':
          tmp->data->leaf_element->number.val = (long long)((unsigned char)'\r');
          break;
        case 't':
          tmp->data->leaf_element->number.val = (long long)((unsigned char)'\t');
          break;
        case '0':
          tmp->data->leaf_element->number.val = 0;
          break;
        default:
          break;
      }
  ...
  for (i=0;i<length*2-1;i++){
    ...
    if (i%2==0){
      ...
      if ($1[i/2+1] == '\\'){
        i+=2;
        tmp->data->leaf_element->number.offset = T0;
        tmp->data->leaf_element->number.val = (long long)((unsigned char)$1[i/2+1]);
        switch ($1[i/2+1]){
          case 'n':
            tmp->data->leaf_element->number.val = (long long)((unsigned char)'\n');
            break;
      ...
      char_end_column = @1.first_column+i/2+1; // TODO: does this belong into the else-block above or not
    }else{
...
Dataatom:		CONSTANT { $$=(DataAtom*)malloc(sizeof(DataAtom)); $$->destination_label = 0; $$->number.offset = $1.offset; $$->number.val = $1.val; $$->operand_label = 0; $$->code_position.first_column = @1.first_column; COPY_CODE_POSITION($$->code_position,@1,@1);}
				| IDENTIFIER { $$=(DataAtom*)malloc(sizeof(DataAtom)); $$->destination_label = $1; $$->number.offset = T0; $$->number.val = 0; $$->operand_label = 0; COPY_CODE_POSITION($$->code_position,@1,@1);}
				| R_PREFIXED_IDENTIFIER { $$=(DataAtom*)malloc(sizeof(DataAtom)); $$->destination_label = $1; $$->number.offset = T0; $$->number.val = 1; $$->operand_label = 0; COPY_CODE_POSITION($$->code_position,@1,@1);}
				| U_PREFIXED_IDENTIFIER IDENTIFIER { $$=(DataAtom*)malloc(sizeof(DataAtom)); $$->destination_label = $1; $$->number.offset = T0; $$->number.val = 0; $$->operand_label = $2; COPY_CODE_POSITION($$->code_position,@1,@2);};
